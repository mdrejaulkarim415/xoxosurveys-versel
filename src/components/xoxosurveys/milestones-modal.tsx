    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6 pb-4 border-b border-[#E2EAF1]">
          ...
          {/* Progress summary */}
          <div className="mt-4 bg-[#F9FAFB] rounded-[10px] p-3 flex items-center justify-between">
            <div>
              <p className="text-[12px] text-[#999999]">Surveys Completed</p>
              <p className="text-[20px] font-bold text-[#36383A]">{state.user.surveysCompleted}<span className="text-[14px] text-[#999999] font-normal">/{state.user.surveyTarget}</span></p>
            </div>
            <div className="flex-1 mx-4">
              <div className="w-full bg-[#E2EAF1] rounded-full h-[8px] overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{
                    width: `${Math.min((state.user.surveysCompleted / state.user.surveyTarget) * 100, 100)}%`,
            ...
            <div className="text-right">
              <p className="text-[12px] text-[#999999]">Next Reward</p>
              <p className="text-[16px] font-bold text-[#0FBCC0]">
                {milestones.find(m => m.canClaim) ? `$${milestones.find(m => m.canClaim)!.reward.toFixed(2)}` : milestones.find(m => !m.isCompleted) ? `$${milestones.find(m => !m.isCompleted)!.reward.toFixed(2)}` : 'All claimed!'}
              </p>
            </div>
          </div>
        </div>
