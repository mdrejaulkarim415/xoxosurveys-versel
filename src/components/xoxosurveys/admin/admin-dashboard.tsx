...
export function AdminDashboard({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      {/* Account Review Alert Banner */}
      {stats.pendingReviews > 0 && (
        <Card className="border-[#D97706]/30 bg-[#FFF7ED] shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-[14px] font-bold text-[#D97706]">Account Review Required</h3>
                <p className="text-[12px] text-[#555555]">
                  {stats.pendingReviews} account{stats.pendingReviews > 1 ? 's' : ''} pending review. Review them now to prevent user frustration.
                </p>
              </div>
              <Badge className="bg-[#D97706] text-white hover:bg-[#D97706] text-[12px]">
            ...
            {stats.pendingReviewsList.length > 0 && (
              <div className="mt-3 space-y-2">
                {stats.pendingReviewsList.map((review) => (
                  <div key={review.id} className="flex items-center justify-between bg-white/60 rounded-[8px] px-3 py-2">
                    <div className="flex items-center gap-2">
                      <AlertCircle size={14} className="text-[#D97706]" />
                      <span className="text-[12px] font-medium text-[#1A1A1A]">{review.user}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#FFF7ED] text-[#D97706] border border-[#F59E0B]/30">{review.reason}</span>
                    </div>
                    <span className="text-[11px] text-[#999999]">{review.time}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
        {statCards.map((card) => (
          <Card
            key={card.title}
            className="border-[#E5E7EB] shadow-sm cursor-pointer hover:shadow-md hover:border-[#0FBCC0]/30 transition-all"
              ...
              {loading ? (
                ...
                stats.pendingCashoutsList.map((co) => (
                  <div key={co.id} className="flex items-center gap-3 p-3 rounded-[8px] bg-[#FAFAFA] hover:bg-[#F0F2F5] transition-colors">
                    ...
                    <Button size="sm" variant="outline" className="text-[11px] h-7 border-[#2DD9B6] text-[#2DD9B6] hover:bg-[#2DD9B6] hover:text-white" onClick={() => onNavigate?.('cashouts')}>
                      Approve
                    </Button>
                  </div>
                ))
      ...
      <Card className="border-[#E5E7EB] shadow-sm">
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Approve Cashouts', icon: <CheckCircle size={20} />, color: '#10B981', bg: '#ECFDF5', page: 'cashouts' },
              { label: 'Review Flagged', icon: <AlertTriangle size={20} />, color: '#F59E0B', bg: '#FFF7ED', page: 'reviews' },
              { label: 'Manage Chargebacks', icon: <RotateCcw size={20} />, color: '#7C3AED', bg: '#F5F3FF', page: 'fraud' },
              { label: 'Export Report', icon: <TrendingUp size={20} />, color: '#22B9CF', bg: '#E0F7FA', page: 'settings' },
            ].map((action) => (
              <button
                key={action.label}
                onClick={() => onNavigate?.(action.page)}
                className="flex flex-col items-center gap-2 p-4 rounded-[10px] border border-[#E5E7EB] hover:shadow-md transition-all bg-white cursor-pointer hover:border-[#0FBCC0]/30"
              >
                <div
