...
export function AdminSettings() {
  ...
  const autoSaveSetting = async <K extends keyof SettingsState>(key: K, value: SettingsState[K]) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Auto-save server error:', res.status, data)
      } else {
        setSaved(true)
  ...
  const handleSave = async () => {
    try {
      setSaving(true)
      // Client-side validation: sanitize API URL fields
      const sanitized = { ...settings }
      if (sanitized.featuredOfferApiUrl && !sanitized.featuredOfferApiUrl.match(/^https?:\/\/.+/)) {
        sanitized.featuredOfferApiUrl = ''
      }
      // Client-side validation: sanitize API key field
      if (sanitized.featuredOfferApiKey && (sanitized.featuredOfferApiKey.trim().length < 20 || !/^[a-zA-Z0-9]+$/.test(sanitized.featuredOfferApiKey.trim()))) {
        sanitized.featuredOfferApiKey = ''
      }
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        console.error('Save settings server error:', res.status, data)
          ...
          <Card className="border-[#E5E7EB] shadow-sm">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="border-t border-[#E5E7EB] pt-5">
                ...
                {/* Toggle */}
                <div className="flex items-center justify-between p-3 bg-[#FFF8E1] rounded-[8px] border border-[#F59E0B]/20 mb-3">
                  <div>
                    <p className="text-[13px] font-medium text-[#92400E]">Show Announcement Banner</p>
                    <p className="text-[11px] text-[#B8860B]">
                      {settings.bannerEnabled
                        ? 'Banner is visible to all users at the top of the dashboard'
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) => autoSaveSetting('bannerEnabled', v)}
                  />
                </div>
                ...
                {settings.bannerEnabled && (
                  <div className="space-y-3 pl-1">
                    <div>
                      <Label className="text-[12px] font-medium">Banner Message</Label>
                      <Textarea
                      ...
                      />
                      <p className="text-[10px] text-[#999999] mt-0.5">This message will be shown to all users at the top of the dashboard</p>
                    </div>
                    <div>
                      <Label className="text-[12px] font-medium">Banner Style</Label>
                      <div className="flex items-center gap-2 mt-1.5">
                        {[
                          { key: 'info', label: 'Info', color: '#0FBCC0' },
                          { key: 'warning', label: 'Warning', color: '#F59E0B' },
                          { key: 'success', label: 'Success', color: '#10B981' },
                          { key: 'error', label: 'Alert', color: '#EF4444' },
                        ].map(type => (
                          <button
                            key={type.key}
                            onClick={() => updateSetting('bannerType', type.key)}
                            className={`px-3 py-1.5 rounded-full text-[11px] font-semibold border-2 transition-all ${
                              settings.bannerType === type.key
                                ? 'text-white border-transparent'
                                : 'bg-white text-[#666] border-[#E5E7EB] hover:border-[#999]'
                            }`}
                            style={settings.bannerType === type.key ? { background: type.color } : undefined}
                          >
                            {type.label}
                          </button>
                        ))}
                    ...
                    {settings.bannerMessage && (
                      <div>
                        ...
                        <div className="mt-1.5">
                          <BannerPreview message={settings.bannerMessage} type={settings.bannerType} />
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="border-t border-[#E5E7EB] pt-5">
                <h4 className="text-[13px] font-bold text-[#1A1A1A] flex items-center gap-2 mb-3">
                  <MessageCircle size={14} className="text-[#0088cc]" /> Support Contact
                </h4>
                <div>
                  <Label className="text-[13px] font-medium">Telegram Support Username</Label>
                  <div className="flex items-center gap-2 mt-1.5 max-w-[360px]">
                    <span className="text-[13px] text-[#999] font-medium h-9 flex items-center px-3 bg-[#F8FAFB] border border-[#E5E7EB] rounded-[6px]">@</span>
                    <Input
                      value={settings.telegramSupportUsername}
                      onChange={(e) => updateSetting('telegramSupportUsername', e.target.value.replace(/^@/, ''))}
                      placeholder="XoXoSurveysSupport"
                      className="h-9 text-[13px] flex-1"
                    />
                  </div>
                  <p className="text-[11px] text-[#999999] mt-1">Telegram username shown to users for support contact (without @)</p>
                  {settings.telegramSupportUsername && (
                    <a
                      href={`https://t.me/${settings.telegramSupportUsername}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-[12px] text-[#0088cc] font-medium mt-1.5 hover:underline"
                    >
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="#0088cc">
                        <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
                      </svg>
                      Preview: t.me/{settings.telegramSupportUsername}
                    </a>
                  )}
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="e.g. Variable, Up to $5.00, etc."
                          className="h-9 text-[13px] mt-1"
                        />
                        ...
                        <Input
                          ...
                          onBlur={(e) => {
                            ...
                            if (val && !val.match(/^https?:\/\/.+/)) {
                              updateSetting('featuredOfferApiUrl', '')
                              alert('API URL must be a valid URL starting with http:// or https://')
                            }
                          }}
                          placeholder="https://revtoo.com/api/offers/"
                          className="h-9 text-[13px] mt-1"
                        />
                        <p className="text-[10px] text-[#999999] mt-0.5">Leave empty to use default RevToo URL. Must start with http:// or https://</p>
                      </div>
                          ...
                          <Input
                            ...
                            onBlur={(e) => {
                              ...
                              if (val && (val.length < 20 || !/^[a-zA-Z0-9]+$/.test(val))) {
                                updateSetting('featuredOfferApiKey', '')
                                alert('API Key must be at least 20 alphanumeric characters (no spaces or special characters). Leave empty to use the default key.')
                              }
                            }}
                            ...
                            type="password"
                          />
                          <p className="text-[10px] text-[#999999] mt-0.5">Min 20 alphanumeric chars. Leave empty to use default key</p>
                        </div>
                        <div>
...
function BannerPreview({ message, type }: { message: string; type: string }) {
  ...
  return (
    ...
    >
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={s.icon} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
        <circle cx="12" cy="12" r="10" />
        <line x1="12" y1="16" x2="12" y2="12" />
        <line x1="12" y1="8" x2="12.01" y2="8" />
      </svg>
      <p className="text-[13px] font-medium" style={{ color: s.text }}>{message}</p>
    </div>
  )
}
