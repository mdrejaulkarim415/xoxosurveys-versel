...
export function AdminCashouts() {
  ...
  // Release reserve dialog state
  const [releaseReserveOpen, setReleaseReserveOpen] = useState(false)
  const [releaseReserveCashout, setReleaseReserveCashout] = useState<CashoutRecord | null>(null)
  const [releaseReserveSuccess, setReleaseReserveSuccess] = useState(false)
  ...
  const fetchCashouts = useCallback(async () => {
    ...
    try {
      ...
      const mapped: CashoutRecord[] = (data.cashouts || []).map((c: Record<string, unknown>) => ({
        ...
        reserveAmount: typeof c.reserveAmount === 'number' ? c.reserveAmount : parseFloat(String(c.reserveAmount || 0)),
        reserveStatus: (c.reserveStatus as string) || 'held',
      }))
  ...
  const totalReserved = cashouts.filter((c) => c.reserveAmount > 0 && c.reserveStatus === 'held').reduce((sum, c) => sum + c.reserveAmount, 0)
  const heldReserveCount = cashouts.filter((c) => c.reserveAmount > 0 && c.reserveStatus === 'held').length

  // Release reserve handler
  const handleReleaseReserve = async () => {
    if (!releaseReserveCashout) return
    setActionLoading(releaseReserveCashout.id)
    try {
      const res = await fetch(`/api/admin/cashouts/${releaseReserveCashout.id}/release-reserve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      })
      if (!res.ok) throw new Error('Failed to release reserve')
      setReleaseReserveSuccess(true)
      await fetchCashouts()
      setTimeout(() => {
        setReleaseReserveOpen(false)
        setReleaseReserveSuccess(false)
      }, 2000)
    } catch (err) {
  ...
  const openReleaseReserveDialog = (cashout: CashoutRecord) => {
    setReleaseReserveCashout(cashout)
    setReleaseReserveSuccess(false)
    setReleaseReserveOpen(true)
  }

  const getReserveStatusBadge = (status: string) => {
    switch (status) {
      case 'held': return <Badge className="bg-[#FFF7ED] text-[#D97706] hover:bg-[#FFF7ED]"><Clock size={10} className="mr-1" />Held</Badge>
      case 'released': return <Badge className="bg-[#ECFDF5] text-[#10B981] hover:bg-[#ECFDF5]"><CheckCircle size={10} className="mr-1" />Released</Badge>
      case 'forfeited': return <Badge className="bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEF2F2]"><XCircle size={10} className="mr-1" />Forfeited</Badge>
      default: return <Badge variant="secondary">{status}</Badge>
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card
          className="border-[#D97706]/30 shadow-sm cursor-pointer hover:border-[#D97706]/60 hover:shadow-md transition-all"
        ...
        >
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-[12px] text-[#999999]">Total Reserved</p>
              <p className="text-[22px] font-bold text-[#D97706]" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>${totalReserved.toFixed(2)}</p>
              <p className="text-[10px] text-[#999999]">{heldReserveCount} held · Click to manage</p>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform ${showReservePanel ? 'rotate-180' : ''}`}>
      ...
      {/* Reserve Management Panel */}
      {showReservePanel && (
        <Card className="border-[#D97706]/30 shadow-md" style={{ background: 'linear-gradient(135deg, #FFFBEB 0%, #FFF7ED 100%)' }}>
          <CardHeader className="pb-2">
            <CardTitle className="text-[16px] font-bold text-[#D97706] flex items-center gap-2" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
              <DollarSign size={18} /> Reserve Management
              <Badge className="bg-[#D97706] text-white hover:bg-[#D97706] ml-2">{heldReserveCount} held</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {cashouts.filter(c => c.reserveAmount > 0 && c.reserveStatus === 'held').length === 0 ? (
              <div className="text-center py-6">
                <CheckCircle size={32} className="mx-auto text-[#10B981] mb-2" />
                <p className="text-[14px] font-medium text-[#1A1A1A]">No reserves held</p>
                <p className="text-[12px] text-[#999999]">All reserves have been released or forfeited</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {cashouts
                  .filter(c => c.reserveAmount > 0 && c.reserveStatus === 'held')
                  .map(cashout => (
                    <div key={cashout.id} className="flex items-center gap-3 p-3 bg-white rounded-[10px] border border-[#E5E7EB] hover:border-[#D97706]/40 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-[#1A1A1A] truncate">{cashout.userEmail}</p>
                        <p className="text-[11px] text-[#999999]">
                          {cashout.giftCardType} · ${cashout.amount.toFixed(2)} withdrawal · {new Date(cashout.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-[16px] font-bold text-[#D97706]" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
                          ${cashout.reserveAmount.toFixed(2)}
                        </p>
                        <p className="text-[10px] text-[#999999]">reserved</p>
                      </div>
                      <Button
                        size="sm"
                        className="h-8 text-[11px] text-white flex-shrink-0"
                        style={{ background: '#D97706' }}
                        disabled={releaseLoading === cashout.id}
                        onClick={async () => {
                          setReleaseLoading(cashout.id)
                          try {
                            const res = await fetch(`/api/admin/cashouts/${cashout.id}/release-reserve`, {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                            })
                            if (!res.ok) throw new Error('Failed to release reserve')
                      ...
                      >
                        {releaseLoading === cashout.id ? (
                          <Loader2 size={12} className="animate-spin" />
                        ) : (
                          <><CheckCircle size={12} className="mr-1" /> Release</>
                        )}
                      </Button>
                    </div>
                  ))}
                <div className="flex items-center justify-between pt-2 mt-2 border-t border-[#D97706]/20">
                  <p className="text-[12px] text-[#78350F]">
                    Total: <span className="font-bold">${totalReserved.toFixed(2)}</span> held across {heldReserveCount} cashout{heldReserveCount !== 1 ? 's' : ''}
                  </p>
                  {heldReserveCount > 1 && (
                    <Button
                      size="sm"
                      className="h-8 text-[11px] text-white"
                      style={{ background: '#D97706' }}
                      disabled={releaseLoading === 'bulk'}
                      onClick={async () => {
                        setReleaseLoading('bulk')
                        const heldCashouts = cashouts.filter(c => c.reserveAmount > 0 && c.reserveStatus === 'held')
                        try {
                          await Promise.all(
                            heldCashouts.map(c =>
                              fetch(`/api/admin/cashouts/${c.id}/release-reserve`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                              })
                            )
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] text-[#555555]">
                  ...
                  {parseFloat(chargebackRate) > 5 && ' Chargeback rate is above 5% - review your fraud prevention settings.'}
                </p>
              </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-b border-[#E5E7EB]">
                        ...
                        <TableHead className="text-[12px] text-[#999999]">Reserve</TableHead>
                    ...
                    <TableBody>
                      {loading ? (
                        <TableRow>
                          <TableCell colSpan={tab === 'pending' ? 11 : 10} className="text-center py-12">
                        ...
                        <TableRow key={cashout.id} className={`border-b border-[#F0F2F5] hover:bg-[#FAFAFA] ${cashout.isChargeback ? 'bg-[#F5F3FF]/50' : ''}`}>
                          ...
                          <TableCell className="text-[11px]">
                            {cashout.reserveAmount > 0 ? (
                              <div className="flex flex-col gap-0.5">
                                <span className="text-[12px] font-bold text-[#D97706]">${cashout.reserveAmount.toFixed(2)}</span>
                                {getReserveStatusBadge(cashout.reserveStatus)}
                              </div>
                            ) : (
                                ...
                                <>
                                  ...
                                  {(cashout.status === 'approved' || cashout.status === 'processed') && !cashout.isChargeback && (
                                    <>
                                      ...
                                      {cashout.reserveAmount > 0 && cashout.reserveStatus === 'held' && (
                                        <Button
                                          size="sm"
                                          variant="outline"
                                          className="h-7 text-[11px] border-[#D97706] text-[#D97706] hover:bg-[#D97706] hover:text-white"
                                          title="Release Reserve"
                                          onClick={() => openReleaseReserveDialog(cashout)}
                                        >
                                          <DollarSign size={12} className="mr-1" /> Release
                                        </Button>
                                      )}
                                      <Button
                                        size="icon"
      ...
      {/* Release Reserve Dialog */}
      <Dialog open={releaseReserveOpen} onOpenChange={setReleaseReserveOpen}>
        <DialogContent className="max-w-[480px]">
          {releaseReserveCashout && (
            <>
              <DialogHeader>
                <DialogTitle className="text-[18px] font-bold text-[#D97706]" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
                  <div className="flex items-center gap-2">
                    <DollarSign size={20} />
                    Release Reserve
                  </div>
              ...
              {releaseReserveSuccess ? (
                <div className="text-center py-6">
                  ...
                  <h3 className="text-[16px] font-bold text-[#1A1A1A] mb-1">Reserve Released!</h3>
                  <p className="text-[13px] text-[#999999]">
                    ${releaseReserveCashout.reserveAmount.toFixed(2)} has been released back to the user&apos;s available balance.
                  </p>
                </div>
              ) : (
                <>
                  {/* Cashout Info */}
                  <div className="flex items-center gap-3 p-3 rounded-[10px] border border-[#D97706]/20" style={{ background: '#FFF7ED' }}>
                    <div className="w-[40px] h-[40px] rounded-full bg-[#D97706] flex items-center justify-center flex-shrink-0">
                      <DollarSign size={18} className="text-white" />
                    </div>
                    <div className="flex-1">
                      <p className="text-[14px] font-semibold text-[#1A1A1A]">{releaseReserveCashout.userEmail}</p>
                      <p className="text-[12px] text-[#555555]">
                        {releaseReserveCashout.giftCardType} · ${releaseReserveCashout.amount.toFixed(2)} withdrawal
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-[18px] font-bold text-[#D97706]" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
                        ${releaseReserveCashout.reserveAmount.toFixed(2)}
                      </p>
                      <p className="text-[11px] text-[#999999]">Reserve Held</p>
                    </div>
                  </div>

                  {/* Warning */}
                  <div className="p-3 bg-[#FFFBEB] rounded-[8px] border border-[#D97706]/20">
                    <div className="flex items-start gap-2">
                      <AlertCircle size={16} className="text-[#D97706] mt-0.5 flex-shrink-0" />
                      <p className="text-[12px] text-[#78350F]">
                        Releasing the reserve will add ${releaseReserveCashout.reserveAmount.toFixed(2)} back to the user&apos;s available balance.
                        The reserve was held as security for this cashout. Make sure the cashout has been properly processed before releasing.
                      </p>
                    </div>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={actionLoading === releaseReserveCashout.id}
                    className="w-full h-[44px] text-white disabled:opacity-50"
                    style={{ background: '#D97706' }}
                  >
                    {actionLoading === releaseReserveCashout.id ? (
                      <Loader2 size={16} className="mr-2 animate-spin" />
                    ) : (
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setReleaseReserveOpen(false)}
