...
export type AdminPage = 'dashboard' | 'users' | 'survey-walls' | 'surveys' | 'fraud' | 'cashouts' | 'reviews' | 'support' | 'settings'

const navItems: { id: AdminPage; label: string; icon: React.ReactNode }[] = [
  { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={20} /> },
  { id: 'reviews', label: 'Reviews', icon: <UserCheck size={20} /> },
...
export function AdminLayout() {
  ...
  // Fetch pending reviews count for notification badge
  useEffect(() => {
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'dashboard': return <AdminDashboard onNavigate={(page) => setActivePage(page as AdminPage)} />
          ...
          {navItems.map((item) => (
            ...
            >
              ...
              {item.id === 'reviews' && pendingReviewsCount > 0 && (
                <span className="ml-auto min-w-[20px] h-5 bg-[#EF4444] rounded-full flex items-center justify-center text-[10px] font-bold text-white px-1.5">
                  {pendingReviewsCount > 9 ? '9+' : pendingReviewsCount}
                </span>
              )}
              {activePage === item.id && item.id !== 'reviews' && <ChevronRight size={16} className="ml-auto opacity-70" />}
            </button>
          ))}
