...
/**
 * Validate that an API key looks like a real API key (not a password, email, etc.)
 * RevToo API keys are long alphanumeric strings (typically 20+ chars, no spaces).
 * Common invalid values: "admin123", passwords, emails, etc.
 */
function isValidApiKey(key: string): boolean {
  if (!key || key.trim() === '') return false
  const trimmed = key.trim()
  // Must be at least 20 chars (real API keys are long)
  if (trimmed.length < 20) return false
  // Must be alphanumeric only (no @, spaces, or special chars that passwords might have)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawApiKey = settingsMap.featuredOfferApiKey || ''
    let apiKey = isValidApiKey(rawApiKey) ? rawApiKey : ''
    let apiUrl = settingsMap.featuredOfferApiUrl || ''

    // If no valid custom API settings, try SurveyWall config
    if (!apiKey || !apiUrl) {
      const revtooWall = await db.surveyWall.findFirst({
        where: { provider: 'revtoo' },
      })
      if (!apiKey) {
        // Also validate the SurveyWall API key
        apiKey = (revtooWall?.apiKey && isValidApiKey(revtooWall.apiKey))
          ? revtooWall.apiKey
          : DEFAULT_REVTOO_API_KEY
      }
      if (!apiUrl) {
    ...
    // Validate apiUrl is a valid URL (prevent email or other garbage from being used)
    try {
      new URL(apiUrl)
    } catch {
      console.warn('[Revtoo Offer] Invalid API URL in settings, falling back to default:', apiUrl)
      apiUrl = DEFAULT_REVTOO_API_URL
      apiKey = DEFAULT_REVTOO_API_KEY
    }

    // Log if we're using defaults because custom values were invalid
    if (rawApiKey && !isValidApiKey(rawApiKey)) {
      console.warn('[Revtoo Offer] Custom API key looks invalid (too short or contains special chars), using default. Value was:', rawApiKey.substring(0, 4) + '...')
    }
