...
export async function POST(request: NextRequest) {
  try {
    ...
    // 9. Create "New Login Detected" notification
    try {
      // Get country/city from IP if available
      let location = 'Unknown location'
      try {
        const ipRecord = await db.userIp.findUnique({ where: { userId_ipAddress: { userId: user.id, ipAddress } } })
        if (ipRecord) {
          const parts = [ipRecord.city, ipRecord.country].filter(Boolean)
      ...
      await db.notification.create({
        data: {
          ...
          message: `Your account was logged in from ${location} (IP: ${ipAddress})`,
          iconType: 'login',
          metadata: JSON.stringify({ ipAddress, userAgent: request.headers.get('user-agent'), location }),
        },
      })
    } catch (e) {
      console.warn('[Login] Notification creation failed:', e)
    }
