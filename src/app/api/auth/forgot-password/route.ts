 ...
 * The passwordResetToken and passwordResetExpires columns are now
 * declared directly in the Prisma schema, so no runtime ALTER TABLE is needed.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find user by email using Prisma (works with both SQLite and PostgreSQL)
    ...
    // Save token to user record using Prisma
