...
export async function GET() {
  try {
    ...
    // ============ Reserve Stats ============
    const totalReservedBalanceAgg = await db.user.aggregate({
      _sum: { reservedBalance: true },
    })
    const totalReservedBalance = totalReservedBalanceAgg._sum.reservedBalance || 0
    ...
    const pendingReviews = await db.accountReview.count({ where: { status: 'pending' } })
    const recentReviews = await db.accountReview.findMany({
      where: { status: 'pending' },
      orderBy: { createdAt: 'desc' },
      take: 5,
      select: {
    ...
    const pendingReviewsList = recentReviews.map((r) => ({
      id: r.id,
      user: r.user.firstname && r.user.lastname ? `${r.user.firstname} ${r.user.lastname}` : r.user.email,
      reason: r.reason === 'auto_4dollar' ? 'Auto ($4 Earned)' : r.reason === 'manual_admin' ? 'Manual Admin' : r.reason === 'fraud_suspicion' ? 'Fraud Suspicion' : r.reason,
