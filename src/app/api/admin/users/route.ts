...
export async function POST(request: Request) {
  try {
    ...
    // Generate a unique numeric userId (start from 1000+)
    const lastUser = await db.user.findFirst({ orderBy: { userId: 'desc' }, select: { userId: true } })
    const nextUserId = (lastUser?.userId ?? 99) + 1
